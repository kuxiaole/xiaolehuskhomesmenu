...
public final class TpaMenuCommand implements CommandExecutor, TabCompleter {
    ...
    public TpaMenuCommand(TpaMenuPlugin plugin, TpaMenuService menuService) {
        this.plugin = plugin;
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (!player.hasPermission("tpamenu.open")) {
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (args.length == 1 && sender.hasPermission("tpamenu.reload") && "reload".startsWith(args[0].toLowerCase())) {
